...
def size_lumin_intrinsic(hlrs, lumins, w, com_comp, diff_comp, com_ncomp,
    ...
    # Define okinds
    okinds = np.logical_or(com_comp, diff_comp)

    if w.size <= 1:
        return

    fig = plt.figure(figsize=(1.7 * 3.5, 1.2 * 3.5))
    gs = gridspec.GridSpec(2, 4, height_ratios=(4, 20),
                           width_ratios=(20, 4, 1, 1))
    ...
    Hbot_all, bin_edges = np.histogram(lumins[okinds], bins=lumin_bins)
    ...
    Htop_all, bin_edges = np.histogram(hlrs[okinds], bins=hmr_bins)
