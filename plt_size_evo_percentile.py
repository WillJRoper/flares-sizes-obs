...
for reg, snap in reg_snaps:
    ...
    for f in filters:
        ...
        for r in [0.2, 0.5, 0.8]:
            hlr_dict[snap][f].setdefault(str(r), []).extend(
                orientation_group[f]["HLR_%.1f" % r][...][okinds])
            hlr_app_dict[snap][f].setdefault(str(r), []).extend(
                orientation_group[f]["HLR_Aperture_%.1f" % r][...][okinds])
            hlr_pix_dict[snap][f].setdefault(str(r), []).extend(
                orientation_group[f]["HLR_Pixel_%.1f" % r][...][okinds])
    ...
    for f in filters:
        ...
        print(reg, snap, f, masses[okinds].size)
        for r in [0.2, 0.5, 0.8]:
            intr_hlr_dict[snap][f].setdefault(str(r), []).extend(
                orientation_group[f]["HLR_%.1f" % r][...][okinds])
            intr_hlr_app_dict[snap][f].setdefault(str(r), []).extend(
                orientation_group[f]["HLR_Aperture_%.1f" % r][...][okinds])
            intr_hlr_pix_dict[snap][f].setdefault(str(r), []).extend(
                orientation_group[f]["HLR_Pixel_%.1f" % r][...][okinds])
...
for f in filters:
    ...
    for snap in snaps:
        ...
        quants = weighted_quantile(np.array(hlr_dict[snap][f]["0.5"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        hlr_med.append(quants[1])
        
        quants = weighted_quantile(np.array(hlr_dict[snap][f]["0.2"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        hlr_16.append(quants[1])
        
        quants = weighted_quantile(np.array(hlr_dict[snap][f]["0.8"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        hlr_84.append(quants[1])

        quants = weighted_quantile(np.array(hlr_app_dict[snap][f]["0.5"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        hlr_med_app.append(quants[1])
        
        quants = weighted_quantile(np.array(hlr_app_dict[snap][f]["0.2"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        hlr_16_app.append(quants[1])
        
        quants = weighted_quantile(np.array(hlr_app_dict[snap][f]["0.8"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        hlr_84_app.append(quants[1])

        quants = weighted_quantile(np.array(hlr_pix_dict[snap][f]["0.5"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        hlr_med_pix.append(quants[1])

        quants = weighted_quantile(np.array(hlr_pix_dict[snap][f]["0.2"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        hlr_16_pix.append(quants[1])

        quants = weighted_quantile(np.array(hlr_pix_dict[snap][f]["0.8"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        hlr_84_pix.append(quants[1])

        quants = weighted_quantile(np.array(intr_hlr_dict[snap][f]["0.5"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        intr_hlr_med.append(quants[1])

        quants = weighted_quantile(np.array(intr_hlr_dict[snap][f]["0.2"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        intr_hlr_16.append(quants[1])

        quants = weighted_quantile(np.array(intr_hlr_dict[snap][f]["0.8"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        intr_hlr_84.append(quants[1])

        quants = weighted_quantile(np.array(intr_hlr_app_dict[snap][f]["0.5"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        intr_hlr_med_app.append(quants[1])

        quants = weighted_quantile(np.array(intr_hlr_app_dict[snap][f]["0.2"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        intr_hlr_16_app.append(quants[1])

        quants = weighted_quantile(np.array(intr_hlr_app_dict[snap][f]["0.8"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        intr_hlr_84_app.append(quants[1])

        quants = weighted_quantile(np.array(intr_hlr_pix_dict[snap][f]["0.5"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        intr_hlr_med_pix.append(quants[1])

        quants = weighted_quantile(np.array(intr_hlr_pix_dict[snap][f]["0.2"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        intr_hlr_16_pix.append(quants[1])

        quants = weighted_quantile(np.array(intr_hlr_pix_dict[snap][f]["0.8"]), [0.16, 0.5, 0.84], sample_weight=w,
                                   values_sorted=False, old_style=False)
        intr_hlr_84_pix.append(quants[1])
